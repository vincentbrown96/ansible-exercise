...
const preNames = ["Vinny", "Azarie", "Yoofi"];
...
async function addName() {
  ...
  if (name.value !== "") {
    const response = await fetch(`http://18.216.223.28:8080/names/${name.value}`, {
      method: "PUT",
      headers: {
...
async function getNames() {
  try {
    const response = await fetch(`http://18.216.223.28:8080/names`);
